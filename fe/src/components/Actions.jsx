...
const Actions = ({ post: post_ }) => {
    ...
    const handleReply = async () => {
        if (!user) return showToast("Error", "You must logged in to reply this post", "error");
    ...
    const handleLikedAndUnLiked = async () => {
        if (!user) return showToast("Error", "You must be logged to like a post", "error");
        if (isLiking) return;
        setIsLiking(true);
        try {
            ...
            if (!liked) {
                //add the id of currentUser to post.likes array 
                setPost({ ...post, likes: [...post.likes, user._id] });
            } else {
                // remove the id of currentUser from posts.likes array
